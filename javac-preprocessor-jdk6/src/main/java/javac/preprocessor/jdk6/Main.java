package javac.preprocessor.jdk6;

import com.sun.tools.javac.main.JDK6SourceFileOption;
...
public class Main {
    ...
    private static void extendSourceTypesSupport(com.sun.tools.javac.main.Main localMain, Set<String> extensions) {
        ...
        for (int i = recognizedOptions.length - 1; i > -1; i--) {
            ...
            if (recognizedOption.getName() == OptionName.SOURCEFILE) {
                recognizedOptions[i] = new JDK6SourceFileOption(localMain, extensions);
                break;
            }
        }
    }
    ...
    private static class FileManager extends DefaultFileManager {
        ...
        @Override
        protected JavaFileObject.Kind getKind(String s) {
            if (preprocessors.keySet().contains(s)) {
                return JavaFileObject.Kind.SOURCE;
            }
            return super.getKind(s);
        }
    }
